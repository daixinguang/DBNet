...
#pragma omp parallel for
	for (size_t i = 0; i < filelists.size(); i++)
	{
		...
		//cout<<i<<"++++++anno_file_name="<< anno_file_name<<endl;
    //cout<<i<<"++++++detect_lanes="<< detect_file_name<<endl;    
		...
		if (show)
		{
			...
			waitKey(0);
                ...
                strcpy(name_lists[i],anno_file_name.c_str());
        ...
        for (auto result: tuple_lists) {
                //cout<<name_lists[idx]<<" tp="<< get<1>(result) << ", fp=" << get<2>(result) << " fn=" << get<4>(result) << endl;
                idx++;
        }
