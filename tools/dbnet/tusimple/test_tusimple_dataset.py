...
from tools.dbnet.common import COLORS


def parse_args():
    parser = argparse.ArgumentParser(description='MMDet test detector')
    parser.add_argument(
        '--config', required=True, help='test config file path')
    parser.add_argument('--show', required=True, help='show results')
    parser.add_argument(
        '--max_show_num', type=int, default=50, help='show results')
...
def vis_one(data):
    ...
    gt_hm = data['gt_hm'].data[0].detach().cpu().numpy()[0, :, :, :] * 255
    ...
    for i, mask_info in enumerate(gt_masks):
        ...
        for p in points:
            cv2.circle(vis_img, (hm_downscale * p[0], hm_downscale * p[1]), 3,
                       color, -1)
            cv2.circle(vis_img, (hm_downscale * p[0], hm_downscale * p[1]), 1,
                       (0, 0, 0), -1)
        img = vis_img
    return img, vis_hm
...
def main():
    ...
    for index, data in enumerate(data_loader):
        file_name = data['img_metas'].data[0][0]['sub_img_name']
