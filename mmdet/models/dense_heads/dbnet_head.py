...
class DBNetHeadFast(nn.Module):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(DBNetHeadFast, self).__init__()
        ...
        self.cfe_head = CtnetHead(
            heads=dict(offset_map=self.joint_nums * 2),
            channels_in=branch_in_channels,
            final_kernel=1,
            head_conv=branch_in_channels)

        self.cfe_tail = CtnetHead(
            heads=dict(offset_map=self.joint_nums * 2),
            channels_in=branch_in_channels,
            final_kernel=1,
            head_conv=branch_in_channels)   

        self.gas_head1 = CtnetHead(
            heads=dict(offset_map=self.joint_nums * 2),
            channels_in=branch_in_channels,
            final_kernel=1,
            head_conv=branch_in_channels)   

        self.gas_head2 = CtnetHead(
            heads=dict(offset_map=self.joint_nums * 2),
            channels_in=branch_in_channels,
            final_kernel=1,
            head_conv=branch_in_channels)   
        self.gas_head3 = CtnetHead(
            heads=dict(offset_map=self.joint_nums * 2),
            channels_in=branch_in_channels,
            final_kernel=1,
            head_conv=branch_in_channels)   
    ...
    def ktdet_decode_fast(self, heat, offset, error, thr=0.1, root_thr=1):
        ...
        mask = torch.lt(offset_split[1], root_thr)   
        mask_nms = torch.gt(heat_nms, thr)   
        ...
        coord_mat = make_coordmat(shape=heat.shape[1:])   
    ...
    def forward_train(self, inputs, aux_feat=None):
        ...
        if aux_feat is not None:
            f_hm = aux_feat
        o = self.cfe_head(f_hm)
        pts_offset = o['offset_map']

        o_end=self.cfe_tail(f_hm)   
        end_offsets=o_end['offset_map']   

        o_1=self.gas_head1(f_hm)   
        pts_offsets1=o_1['offset_map']   
        o_2=self.gas_head2(f_hm)   
        pts_offsets2=o_2['offset_map']   
        o_3=self.gas_head3(f_hm)   
        pts_offsets3=o_3['offset_map']   

        o_ = self.reg_head(f_hm)
        int_offset = o_['offset_map']

        return [cpts_hm, kpts_hm, pts_offset, int_offset,end_offsets,pts_offsets1,pts_offsets2,pts_offsets3]
    ...
    def forward_test(
        ...
        if pts_offset.shape[1] > 2:
            ...
            mask = torch.lt(offset_split[1], self.root_thr)   
            mask_nms = torch.gt(heat_nms, kpt_thr)   
            mask_low = mask * mask_nms
